...
//# hinge_pin_diameter - diameter of the sphere that connects two parts of the hinge together 
...
//
module hinge (hinge_width = 7, hinge_thickness = 1.5, hinge_diameter = 5, hinge_height = 8, hinge_pin_diameter = 2.4, holer = true, hinge_holes = false)
{
    hole_lip = 1;
    translate([0,hinge_width/2,0]) rotate([90,-90,0]) mirror([0,1,0])
        ...
        {
            union() {   
                translate([0,0,hinge_width]) sphere(d=hinge_pin_diameter);
                ...
                if (holer)
                {
                    ...
                    if (hinge_holes) {
                      translate([0,0,-hinge_width]) cylinder( hinge_width*3, d=hinge_pin_diameter);
                    }
                }
            }
            if (!holer)
            {
                ...
                if (hinge_holes) {
                  translate([0,0,-hinge_width/2]) cylinder( hinge_width*2, d=hinge_pin_diameter);
                }
            }
        }
}
